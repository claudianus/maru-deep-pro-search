...
> v0.9.3 이후의 새로운 기능(9엔진, 3계층 레이트 리밋, 안티봇 세션 재사용, 72개 보안 시그니처, Audit Logging, Docker, 21개 에이전트 어댑터, 3계층 실제 강제 아키텍처 등)은 [영문 README](./README.md)를 참조하세요.
...
<p align="center">
  ...
  <a href="https://github.com/claudianus/maru-deep-pro-search/blob/main/tests/"><img src="https://img.shields.io/badge/tests-202%20passing-brightgreen?style=flat-square" alt="Tests"></a>
...
| 기능 | 구현 방식 |
|-----------|-----|
| **검색** | 9개 엔진을 비동기 HTTP로 직접 스크래핑. API 키 불필요. |
...
이건 독립 검색 도구가 아닙니다. **검색 MCP 서버이자 하네스 세팅 도구**입니다 — 검색·페치 MCP 툴을 제공하고, 에이전트 설정에 리서치 우선 규칙을 주입합니다.

- **9엔진 자동 폴오버** — DuckDuckGo (HTML + Lite), Bing, Google, Yahoo, Ecosia, Baidu, Startpage, Naver. 하나가 망가지면 다음이 즉시 이어받습니다.
...
9개 검색 엔진을 지원하며, 모두 직접 스크래핑 방식:

| 엔진 | 방식 | 비고 |
|--------|--------|------|
| DuckDuckGo (lite) | HTML 스크래핑 | 기본, 가장 빠름 |
| DuckDuckGo (html) | HTML 스크래핑 | JS 지원 폴오버 |
| Bing | HTML 스크래핑 | 로케일 고정 (`en-US`) |
| Google | 스텔스 세션 재사용 | 안티봇 회피, 최저 레이트 리밋 위험 |
| Yahoo | HTML 스크래핑 | 리다이렉트 디코딩 |
| Ecosia | HTML 스크래핑 | 오가닉 전용 필터링 |
| Baidu | HTML 스크래핑 | 노이즈 필터링 (`result-op` 제외) |
| Startpage | JS 렌더링 프록시 | Google 프라이버시 프록시 |
| Naver | HTML 스크래핑 (SSR) | 난독화 DOM 복구 |

**레지스트리 패턴**: `SearchEngineRegistry`는 `_instances` 딕셔너리를 사용한 싱글턴 재사용이 있는 팩토리 패턴입니다. 스텔스가 필요한 엔진은 `AsyncStealthySession`을 사용해 브라우저를 재사용하며, 요청마다 새 브라우저를 띄우는 것보다 레이트 리밋 히트를 극적으로 줄입니다.
...
| 전체 `deep_research` | <10초 | 9개 엔진, Semaphore(3) 동시 제한, HIGH 결과 3개 시 조기 중단 |
...
202개 테스트, 모두 통과. 모든 엔진, 랭킹 알고리즘, 콘텐츠 추출, 살균, Harness 지속성, 레이트 리밋, 전체 리서치 파이프라인에 대한 통합 테스트를 포함한 단위 테스트를 커버합니다.
